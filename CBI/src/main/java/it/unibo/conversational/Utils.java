...
public final class Utils {
  ...
  public static void writeParsing(final String folderName, final String fileName, final List<Mapping> sentences) {
    ...
    try {
      ...
      for (final Mapping set : sentences.stream().sorted((Mapping s1, Mapping s2) -> {
                final int s1f = s1.ngrams.stream().anyMatch(n -> n.type.equals(Type.Q)) ? 1 : 0;
                final int s2f = s2.ngrams.stream().anyMatch(n -> n.type.equals(Type.Q)) ? 1 : 0;
  ...
  public static DataType getDataType(final String t) {
    ...
    } else if (t.equals(JDBCType.TINYINT.toString()) || t.equals(JDBCType.SMALLINT.toString()) || t.equals(JDBCType.INTEGER.toString())
